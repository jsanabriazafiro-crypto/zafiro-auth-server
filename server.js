...
res.end(page(‘Inicio’, `<h1>Zafiro Auth Server</h1> <p class="sub">Obtén tus tokens de API para Shopify, QuickBooks y Lightspeed</p> <div class="platform-grid"> <a href="/shopify/start" class="platform-btn">🛍 Shopify</a> <a href="/quickbooks/start" class="platform-btn">📊 QuickBooks</a> <a href="/lightspeed/start" class="platform-btn">⚡ Lightspeed</a> </div>`));
return;
}
...
res.end(page(‘✅ Shopify Conectado’, `<div class="success">🎉 ¡Token de Shopify obtenido exitosamente!</div> <p style="font-size:13px;color:#4a4540;margin-bottom:8px"><strong>Tu Access Token permanente:</strong></p> <div class="token-box"> <button class="copy-btn" onclick="copyText('${data.access_token}')">Copiar</button> ${data.access_token} </div> <div class="info">⚠️ Guarda este token en un lugar seguro. No expira pero solo se muestra una vez aquí.</div> <a href="/" class="btn outline">← Volver al inicio</a>`));
} else {
...
if (!QB_CLIENT_ID) {
res.end(page(‘QuickBooks’, `<h1>QuickBooks</h1> <div class="info">Aún no has configurado las credenciales de QuickBooks.<br>Agrégalas como variables de entorno en Render:<br><strong>QB_CLIENT_ID</strong> y <strong>QB_CLIENT_SECRET</strong></div> <a href="/" class="btn outline">← Volver</a>`));
return;
}
const BASE = process.env.BASE_URL || `http://localhost:${PORT}`;
...
res.end(page(‘✅ QuickBooks’, `<div class="success">🎉 Código de QuickBooks recibido</div> <p style="font-size:13px;color:#4a4540">Código: <code>${code}</code></p> <p style="font-size:13px;color:#4a4540">Realm ID: <code>${realmId}</code></p> <div class="info">Guarda estos valores — los necesitas para intercambiar por el Access Token de QuickBooks.</div> <a href="/" class="btn outline">← Volver</a>`));
return;
}
...
if (!LS_CLIENT_ID) {
res.end(page(‘Lightspeed’, `<h1>Lightspeed</h1> <div class="info">Configura <strong>LS_CLIENT_ID</strong> y <strong>LS_CLIENT_SECRET</strong> en Render.</div> <a href="/" class="btn outline">← Volver</a>`));
return;
}
const BASE = process.env.BASE_URL || `http://localhost:${PORT}`;
...
if (data.access_token) {
res.end(page(‘✅ Lightspeed Conectado’, `<div class="success">🎉 ¡Token de Lightspeed obtenido exitosamente!</div> <p style="font-size:13px;color:#4a4540;margin-bottom:8px"><strong>Access Token:</strong></p> <div class="token-box"> <button class="copy-btn" onclick="copyText('${data.access_token}')">Copiar</button> ${data.access_token} </div> ${data.refresh_token ?`
...
${data.refresh_token}
</div>`: ''} <div class="info">⚠️ Guarda ambos tokens. El Access Token expira en 1 hora — el Refresh Token se usa para renovarlo automáticamente.</div> <p style="font-size:12px;color:#4a4540;margin-top:12px">Añade en Render:<br> <strong>LS_TOKEN</strong> = Access Token<br> <strong>LS_REFRESH_TOKEN</strong> = Refresh Token</p> <a href="/" class="btn outline" style="margin-top:16px">← Volver al inicio</a>`));
} else {
